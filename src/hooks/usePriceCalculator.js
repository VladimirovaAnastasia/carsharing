...
export default function usePriceCalculator() {
    ...
    const calculatePrice = () => {
        if (
            (order?.dateTo || order?.data?.dateTo) &&
            (order?.dateFrom || order.data?.dateFrom) &&
            (rates || order?.rateId)
        ) {
            const rate = rates?.find((item) => item.id === rateId);
            const time = (order.dateTo || order.data.dateTo) - (order.dateFrom || order.data.dateFrom);

            let tempPrice = 0;
            switch (order?.rateId?.unit || order?.data?.rateId?.rateTypeId?.unit) {
                case 'мин': {
                    tempPrice = (rate?.price || order?.data?.rateId?.price) * (time / 1000 / 60);
                    setOrderDuration(`
                ...
                case 'сутки': {
                    tempPrice = (rate?.price || order?.data?.rateId?.price) * Math.ceil(time / 1000 / 60 / 60 / 24);
                    setOrderDuration(`${Math.ceil(time / (1000 * 60 * 60 * 24))}д 0ч`);
                    break;
                }
                case '7 дней':
                    tempPrice = (rate?.price || order?.data?.rateId?.price) * Math.ceil(time / 1000 / 60 / 60 / 24 / 7);
